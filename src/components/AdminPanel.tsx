...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const [taskTakaPerAd, setTaskTakaPerAd] = useState(3);
  const [taskMaxAdsPerDay, setTaskMaxAdsPerDay] = useState(10);
  ...
  const applySettingsData = (data: any) => {
    ...
    setTaskEnabled(data.taskEnabled ?? true);
    setTaskTakaPerAd(data.taskTakaPerAd ?? 3);
    setTaskMaxAdsPerDay(data.taskMaxAdsPerDay ?? 10);
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      const settingsData = {
        ...
        taskTakaPerAd: taskTakaPerAd || 3,
        taskMaxAdsPerDay: taskMaxAdsPerDay || 10,
      };
    ...
    match /{document=**} {
      ...
      allow write: if request.auth != null;
              ...
              <div className="bg-[#1a1a1d] rounded-[20px] p-5 space-y-4 border border-amber-500/20">
                ...
                <div className="bg-[#0d0d10] rounded-[16px] p-4 border border-blue-500/20 space-y-3 mt-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-lg">📋</span>
                      <div>
                        <p className="text-blue-300 text-sm font-black">Tasks System</p>
                        <p className="text-zinc-600 text-[10px]">Ad দেখলে সরাসরি টাকা পাবে</p>
                      </div>
                    </div>
                    <button onClick={() => setTaskEnabled(!taskEnabled)}
                      className={`relative w-12 h-6 rounded-full transition-all ${taskEnabled ? 'bg-blue-500' : 'bg-white/10'}`}>
                      <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${taskEnabled ? 'left-6' : 'left-0.5'}`} />
                    </button>
                  </div>
                  {taskEnabled && (
                    <div className="space-y-3">
                      <div>
                        <label className="block text-xs text-zinc-500 mb-1.5">প্রতি Ad এ কত টাকা দেবে? (৳)</label>
                        <div className="flex items-center gap-3 bg-[#1a1a1d] rounded-[12px] px-4 py-3 border border-blue-500/20">
                          <button onClick={() => setTaskTakaPerAd(Math.max(1, taskTakaPerAd - 1))} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-zinc-400 hover:text-white">−</button>
                          <input type="number" value={taskTakaPerAd} onChange={e => setTaskTakaPerAd(Number(e.target.value))}
                            className="flex-1 bg-transparent text-blue-300 font-black text-xl outline-none text-center" />
                          <button onClick={() => setTaskTakaPerAd(taskTakaPerAd + 1)} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-zinc-400 hover:text-white">+</button>
                        </div>
                      </div>
                      <div>
                        <label className="block text-xs text-zinc-500 mb-1.5">দিনে সর্বোচ্চ কতটা Ad দেখতে পারবে?</label>
                        <div className="flex items-center gap-3 bg-[#1a1a1d] rounded-[12px] px-4 py-3 border border-blue-500/20">
                          <button onClick={() => setTaskMaxAdsPerDay(Math.max(1, taskMaxAdsPerDay - 1))} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-zinc-400 hover:text-white">−</button>
                          <input type="number" value={taskMaxAdsPerDay} onChange={e => setTaskMaxAdsPerDay(Number(e.target.value))}
                            className="flex-1 bg-transparent text-blue-300 font-black text-xl outline-none text-center" />
                          <button onClick={() => setTaskMaxAdsPerDay(taskMaxAdsPerDay + 1)} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-zinc-400 hover:text-white">+</button>
                        </div>
                        <p className="text-zinc-600 text-[10px] mt-1.5">User দিনে সর্বোচ্চ {taskMaxAdsPerDay} টা Ad দেখতে পারবে = ৳{taskMaxAdsPerDay * taskTakaPerAd} আয়</p>
                      </div>
                      <div className="bg-blue-500/5 border border-blue-500/15 rounded-[12px] p-3">
                        <p className="text-zinc-500 text-[10px]">⚠️ Adsgram Block ID উপরের Adsgram section থেকে set করো</p>
                      </div>
                    </div>
                  )}
                </div>
...
const UsersPanel: React.FC<UsersPanelProps> = ({ movieList }) => {
  ...
  useEffect(() => {
    ...
    const fetchRefs = async () => {
      // ✅ Fix: referredBy দিয়ে users খোঁজো (completed referrals)
      const q = query(collection(db, 'users'), where('referredBy', '==', selectedUser.id));
      const snap = await getDocs(q);
      const completedRefs = snap.docs.map(d => ({ id: d.id, ...d.data(), status: 'completed' }));
      
      // ✅ pendingReferrals থেকেও দেখাও (ad দেখার আগে)
      const pq = query(collection(db, 'pendingReferrals'), where('referrerId', '==', selectedUser.id));
      const pSnap = await getDocs(pq);
      const pendingUserIds = pSnap.docs
        .filter(d => !d.data().completed)
        .map(d => d.data().newUserId);
      
      // pending user এর info আনো
      const pendingRefs: any[] = [];
      for (const uid of pendingUserIds) {
        // already in completedRefs হলে skip
        if (completedRefs.find(r => r.id === uid)) continue;
        try {
          const uSnap = await getDocs(query(collection(db,'users'), where('telegramId','==',uid)));
          if (!uSnap.empty) {
            pendingRefs.push({ id: uSnap.docs[0].id, ...uSnap.docs[0].data(), status: 'pending' });
          }
        } catch {}
      }
      
      setReferrals([...completedRefs, ...pendingRefs]);
      
      // ✅ Fix referralCount if mismatch
      const actualCount = completedRefs.length;
      if (actualCount > 0 && (selectedUser.referralCount || 0) !== actualCount) {
        // Auto-fix referralCount in Firestore
        try {
          await updateDoc(doc(db, 'users', selectedUser.id), { referralCount: actualCount });
        } catch {}
      }
    };
    fetchRefs();
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    <div className="bg-[#0d0d10] rounded-[14px] p-3 border border-white/5">
                      <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest mb-2">
                        কাদের Refer করেছে ({referrals.length})
                        {referrals.filter((r:any) => r.status === 'pending').length > 0 && (
                          <span className="ml-2 text-amber-400">({referrals.filter((r:any) => r.status === 'pending').length} pending)</span>
                        )}
                      </p>
                      {referrals.length === 0 ? (
                        <p className="text-zinc-600 text-xs text-center py-3">কোনো referral নেই</p>
                      ) : (
                        <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                          {referrals.map((r:any) => (
                            <div key={r.id} className={`flex items-center justify-between rounded-lg p-2 border ${r.status === 'pending' ? 'bg-amber-500/5 border-amber-500/15' : 'bg-white/5 border-white/5'}`}>
                              <div className="flex items-center gap-2 min-w-0">
                                <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${r.status === 'pending' ? 'bg-amber-400' : 'bg-emerald-500'}`} />
                              ...
                              <div className="text-right flex-shrink-0">
                                <p className={`text-[10px] font-bold ${r.status === 'pending' ? 'text-amber-400' : 'text-emerald-400'}`}>
