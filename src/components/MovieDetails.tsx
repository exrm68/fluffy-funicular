...
const MovieDetails: React.FC<MovieDetailsProps> = ({ movie, onClose, botUsername, channelLink, appSettings }) => {
      ...
      if (useAdsgram) {
        ...
        const tryAdsgram = () => {
          ...
          if (typeof AdController !== 'undefined') {
            ...
            console.warn('Adsgram SDK not found after retries');
            resolve('skipped'); // SDK load হয়নি — coin দেওয়া হবে না
          }
        };
        tryAdsgram();
        return;
  ...
  const handleMovieLockWatchAd = useCallback(async () => {
    ...
    // ✅ Cooldown check — ad শেষ হওয়ার পর 5s wait
    if (lastAdTime > 0 && diff < cooldownMs) {
    ...
    // ✅ Ad শেষ হওয়ার পরে cooldown শুরু
    ...
    const interval = setInterval(() => {
      secs--;
      if (secs <= 0) { clearInterval(interval); setCooldownRemaining(0); }
      else setCooldownRemaining(secs);
    }, 1000);
      ...
      <AnimatePresence>
        {cooldownRemaining > 0 && (
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }}
            className="fixed bottom-28 left-1/2 -translate-x-1/2 z-[300] bg-black/90 backdrop-blur-xl border border-white/10 text-white px-5 py-3 rounded-2xl text-sm font-bold shadow-2xl flex items-center gap-3">
            <div className="w-8 h-8 rounded-full border-2 border-amber-400 flex items-center justify-center">
              <span className="text-amber-400 font-black text-sm">{cooldownRemaining}</span>
            </div>
            <span className="text-zinc-300">পরের Ad এর জন্য অপেক্ষা করুন...</span>
          </motion.div>
        )}
