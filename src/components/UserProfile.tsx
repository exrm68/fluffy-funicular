...
import {
  ...
  Sparkles, ChevronDown, PlayCircle, ListChecks
} from 'lucide-react';
import {
...
const UserProfile: React.FC<UserProfileProps> = ({ onClose, botUsername }) => {
  ...
  const [screen, setScreen]               = useState<'main'|'earn'|'convert'|'withdraw'|'history'|'tasks'>('main');
  // Tasks state
  const [taskAdLoading, setTaskAdLoading]   = useState(false);
  const [taskSettings, setTaskSettings]     = useState({ takaPerAd: 3, maxAdsPerDay: 10, adsgramBlockId: '', enabled: true });
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'settings', 'config'), s => {
      ...
      // ✅ Task settings
      setTaskSettings({
        takaPerAd:     d.taskTakaPerAd    ?? 3,
        maxAdsPerDay:  d.taskMaxAdsPerDay ?? 10,
        adsgramBlockId: d.adsgramBlockId  || '',
        enabled:       d.taskEnabled      ?? true,
      });
    });
    return () => unsub();
  }, []);

  // ── Today task count load ──────────────────────────────────────────────
  useEffect(() => {
    if (!tgUser) return;
    const uid = String(tgUser.id);
    const todayStr = new Date().toDateString();
    const key = `cine_task_${uid}_${todayStr}`;
    const saved = parseInt(localStorage.getItem(key) || '0');
    setTodayTaskCount(saved);
  }, [tgUser]);
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      <AnimatePresence mode="wait">
        ...
        {screen==='main' && (
          <motion.div key="main" variants={pv} initial="initial" animate="animate" exit="exit"
            ...
            <div className="px-4 space-y-2.5 pb-28">
              {[
                { icon: ListChecks, label:'Tasks — টাকা আয়',    sub:'Ad দেখে সরাসরি টাকা পাও',        color:'text-blue-400',   bg:'bg-blue-500/10 border-blue-500/15',    s:'tasks'   as const },
        ...
        {/* ══════════════════════════ TASKS ═══════════════════════════════ */}
        {screen==='tasks' && (
          <motion.div key="tasks" variants={pv} initial="initial" animate="animate" exit="exit"
            className="flex-1 overflow-y-auto no-scrollbar flex flex-col">

            {/* Header */}
            <div className="px-4 py-4 flex items-center gap-3 border-b border-white/5 sticky top-0 bg-[#09090c]/90 backdrop-blur-xl z-20">
              <button onClick={() => setScreen('main')} className="p-2.5 bg-[#1a1a1d] rounded-full">
                <ArrowLeft size={18} />
              </button>
              <div>
                <h2 className="text-lg font-black">Tasks — টাকা আয়</h2>
                <p className="text-zinc-500 text-[11px]">Ad দেখো, সরাসরি Balance এ টাকা পাও</p>
              </div>
            </div>
            ...
            <div className="p-4 space-y-4 pb-28">
              ...
              {/* Today progress */}
              <div className="bg-gradient-to-br from-blue-500/10 to-indigo-500/10 border border-blue-500/20 rounded-[22px] p-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <p className="text-blue-300 text-[10px] font-black uppercase tracking-widest mb-1">আজকের Progress</p>
                    <p className="text-white font-black text-2xl">{todayTaskCount} <span className="text-zinc-500 text-base font-bold">/ {taskSettings.maxAdsPerDay} Ads</span></p>
                  </div>
                  <div className="text-right">
                    <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest mb-1">আজ আয়</p>
                    <p className="text-emerald-400 font-black text-xl">৳{(todayTaskCount * taskSettings.takaPerAd).toFixed(0)}</p>
                  </div>
                </div>
                <div className="h-2 bg-black/40 rounded-full overflow-hidden border border-white/5">
                  <motion.div
                    ...
                    initial={{ width: 0 }}
                    animate={{ width: `${Math.min(100, (todayTaskCount / taskSettings.maxAdsPerDay) * 100)}%` }}
                    transition={{ duration: 0.8 }}
                  />
                </div>
                <p className="text-zinc-600 text-[10px] mt-1.5 text-right">
                  আরো {Math.max(0, taskSettings.maxAdsPerDay - todayTaskCount)} টা Ad বাকি আজ
                </p>
              </div>

              {/* Task cards — Adsgram */}
              {!taskSettings.enabled ? (
                <div className="bg-[#1a1a1d] border border-white/5 rounded-[20px] p-6 text-center">
                  <p className="text-zinc-500 text-sm">Tasks এখন বন্ধ আছে</p>
                </div>
              ) : todayTaskCount >= taskSettings.maxAdsPerDay ? (
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-[20px] p-5 text-center">
                  <p className="text-2xl mb-2">🎉</p>
                  <p className="text-amber-300 font-black text-base">আজকের সব Task শেষ!</p>
                  <p className="text-zinc-500 text-sm mt-1">কাল আবার আসো</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {/* Single task card */}
                  <div className="bg-white rounded-[20px] overflow-hidden shadow-lg">
                    <div className="p-4">
                      <div className="flex items-center gap-3 mb-3">
                        <div className="w-12 h-12 bg-blue-100 rounded-[14px] flex items-center justify-center flex-shrink-0">
                          <PlayCircle size={26} className="text-blue-600" />
                        </div>
                        <div className="flex-1">
                          <p className="text-gray-900 font-black text-base">ভিডিও এড দেখে আয়🤑</p>
                          <p className="text-gray-400 text-xs font-bold uppercase tracking-wider">ADSGRAM</p>
                        </div>
                        <div className="text-right">
                          <p className="text-emerald-600 font-black text-lg">৳{taskSettings.takaPerAd}</p>
                          <p className="text-gray-400 text-[10px]">/ Ad</p>
                        </div>
                      </div>
                      ...
                      <div className="mb-3">
                        <div className="flex justify-between mb-1">
                          <span className="text-gray-500 text-[11px] font-bold uppercase">VIEWS: {todayTaskCount}/{taskSettings.maxAdsPerDay}</span>
                          <span className="text-gray-500 text-[11px]">মোট আয়: ৳{taskSettings.maxAdsPerDay * taskSettings.takaPerAd}</span>
                        </div>
                        <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-blue-500 rounded-full transition-all duration-500"
                            style={{ width: `${Math.min(100, (todayTaskCount / taskSettings.maxAdsPerDay) * 100)}%` }}
                          />
                        </div>
                      </div>
                      ...
                      {/* Ad button */}
                      {taskMsg ? (
                        <div className={`w-full py-3 rounded-[14px] text-center font-black text-sm mb-1 ${taskMsg.startsWith('✅') ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'}`}>
                          {taskMsg}
                        </div>
                      ) : null}
                      ...
                      <motion.button
                        whileTap={{ scale: 0.97 }}
                        disabled={taskAdLoading || todayTaskCount >= taskSettings.maxAdsPerDay}
                        onClick={async () => {
                          if (!tgUser) { setTaskMsg('❌ Telegram এ login করুন'); setTimeout(() => setTaskMsg(''), 3000); return; }
                          if (!taskSettings.adsgramBlockId) { setTaskMsg('❌ Admin এ Adsgram Block ID set করুন'); setTimeout(() => setTaskMsg(''), 3000); return; }

                          setTaskAdLoading(true);
                          setTaskMsg('');
                          ...
                          try {
                            await new Promise<void>((resolve, reject) => {
                              ...
                              const tryAd = () => {
                                ...
                                if (typeof AdController !== 'undefined') {
                                  try {
                                    // @ts-ignore
                                    AdController.init({ blockId: String(taskSettings.adsgramBlockId) })
                                      .show()
                                      .then(() => resolve())
                            ...
                            // ✅ Ad watched — টাকা দাও
                            const uid = String(tgUser.id);
                            const taka = taskSettings.takaPerAd;
                            const { updateDoc, doc: fsDoc, increment: fsIncrement, addDoc: fsAddDoc, collection: fsColl, serverTimestamp: fsSTS } = await import('firebase/firestore');
                            const { db: fsDb } = await import('../firebase');

                            await updateDoc(fsDoc(fsDb, 'users', uid), { takaBalance: fsIncrement(taka) });
                            await fsAddDoc(fsColl(fsDb, `users/${uid}/coinHistory`), {
                              type: 'earn',
                              reason: `📺 Task Ad Watch`,
                              amount: taka,
                              isTaka: true,
                              createdAt: fsSTS(),
                            });

                            // localStorage এ today count save
                            const todayStr = new Date().toDateString();
                            const key = `cine_task_${uid}_${todayStr}`;
                            const newCount = todayTaskCount + 1;
                            localStorage.setItem(key, String(newCount));
                            setTodayTaskCount(newCount);
                            haptic('success');
                            setTaskMsg(`✅ +৳${taka} Balance এ যোগ হয়েছে!`);
                            setTimeout(() => setTaskMsg(''), 3000);

                          } catch(e) {
                            setTaskMsg('❌ Ad skip করলে টাকা পাবে না');
                            setTimeout(() => setTaskMsg(''), 3000);
                          }
                          setTaskAdLoading(false);
                        }}
                        className={`w-full py-3.5 rounded-[14px] font-black text-sm flex items-center justify-center gap-2 transition-all ${
                          todayTaskCount >= taskSettings.maxAdsPerDay
                      ...
                      >
                        {taskAdLoading ? (
                          ...
                          <><PlayCircle size={18} />▶ অ্যাড দেখুন</>
                        )}
                      </motion.button>
                    </div>
                  </div>
              ...
              {/* Info box */}
              <div className="bg-[#1a1a1d] border border-white/5 rounded-[20px] p-4 space-y-2.5">
                <p className="text-zinc-400 text-xs font-black uppercase tracking-widest">কিভাবে কাজ করে</p>
                {[
                  { icon: '▶️', text: `"অ্যাড দেখুন" বাটনে click করো` },
                  { icon: '📺', text: 'পুরো Ad দেখো — skip করলে টাকা নেই' },
                  { icon: '💰', text: `প্রতি Ad এ ৳${taskSettings.takaPerAd} সরাসরি Balance এ যাবে` },
                  { icon: '🔄', text: `দিনে সর্বোচ্চ ${taskSettings.maxAdsPerDay} টা Ad দেখতে পারবে` },
                  { icon: '💸', text: 'Balance জমলে Withdraw করো bKash/Nagad এ' },
                ].map((item, i) => (
